...
BEGIN
    ...
    RAISE NOTICE 'Nombre actual: auditlog_example - DEBE SER CAMBIADO';
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_database WHERE datname = 'auditlog_example') THEN
        RAISE NOTICE 'Base de datos encontrada: %', 'auditlog_example';
        ...
        WHERE datname = 'auditlog_example' 
        AND pid <> pg_backend_pid();
        -- Esperar un momento para que las conexiones se terminen completamente
        PERFORM pg_sleep(1);
        RAISE NOTICE 'Eliminando base de datos: %', 'auditlog_example';
    ...
    ELSE
        RAISE NOTICE 'La base de datos % no existe, no se requiere eliminación', 'auditlog_example';
    END IF;
...
DROP DATABASE IF EXISTS auditlog_example;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'auditlog_example') THEN
...
-- 1. Backup completo de la base de datos: pg_dump auditlog_example > backup.sql
-- 2. Verificar que estás en el entorno correcto (development/testing)
-- 3. Editar 'auditlog_example' con el nombre real de la BD
